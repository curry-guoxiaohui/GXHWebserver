...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    //printf("m_url:%s\n", m_url);
    const char *p = strrchr(m_url, '/');//  /2CGISQL.cgi

    //处理cgi 2CGISQL.cgi登录 3CGISQL.cgi注册
    if (cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3'))
    {
        ...
        if (*(p + 1) == '3')
        {
            ...
            if (m_users.find(name) == m_users.end())//数据库没有该用户 可以添加
            {
                m_lock.lock();
        ...
        else if (*(p + 1) == '2')
        {
            ...
            // for(map<string,string>::iterator it = m_users.begin();it != m_users.end();it++)
            // {
            //     cout << it->first << ":" << it->second << endl;
            // }
            // cout << (m_users.find(name) != m_users.end() ? "找到了" : "没找到") << endl;
...
{
    return add_content_type() && add_content_length(content_len) && add_linger() &&
           add_blank_line();
}
